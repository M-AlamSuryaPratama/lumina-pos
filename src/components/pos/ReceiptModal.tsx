      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div id="receipt-content" className="bg-white text-black p-6 rounded-lg font-mono text-sm space-y-4">
          <div className="text-center border-b-2 border-dashed border-gray-400 pb-4">
            {logoUrl && <img src={logoUrl} alt={storeName} className="w-16 h-16 mx-auto mb-2 object-contain" />}
            <h2 className="text-xl font-bold tracking-wide">{storeName}</h2>
            {storeAddress && <p className="text-xs text-gray-500 mt-1">{storeAddress}</p>}
            {storePhone && <p className="text-xs text-gray-500">{storePhone}</p>}
            <p className="text-xs text-gray-400 mt-1">Struk Pembayaran</p>
          </div>
